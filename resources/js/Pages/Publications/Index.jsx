...
export default function TwitterStyleFeed({ authUser, publications: initialPublications }) {
    ...
    const [publications, setPublications] = useState(initialPublications.data || initialPublications);
    const [nextPageUrl, setNextPageUrl] = useState(initialPublications.links?.next || initialPublications.next_page_url);
        ...
        >
                ...
                <div className="flex space-x-3">
                    ...
                    <div className="flex-1">
                        <button 
                            ...
                            className="block w-full p-3 text-left rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 transition"
                        >
                            ¿Qué está pasando?
                        </button>
                    </div>
                            ...
                            <div className="flex space-x-3">
                                ...
                                <div className="flex-1">
                                    <textarea
                                        ...
                                        className="w-full border-none focus:ring-0 text-lg placeholder-gray-500 tracking-wide min-h-[50px] resize-none"
                                        autoFocus
                                    />
                                    ...
                                    <div className="mt-4 flex justify-between items-center border-t border-gray-200 pt-4">
                                        <label className="p-2 rounded-full hover:bg-blue-50 text-blue-500 cursor-pointer">
                                        ...
                                        <button
                                            type="submit"
                                            disabled={processing || !data.textContent.trim()}
                                            className={`px-4 py-2 rounded-full font-bold ${!data.textContent.trim() || processing ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'} text-white transition`}
                                        >
            ...
            {publications.length === 0 && !loadingMore ? (
                <div className="py-16 text-center">
                    <PaperAirplaneIcon className="mx-auto h-16 w-16 text-gray-400" />
                ...
                <>
                    <div className="divide-y divide-gray-200">
                        {publications.map((publication) => (
                            <div key={publication.id} className="p-4 hover:bg-gray-50 transition">
                                <div className="flex space-x-3">
                                    <img 
                                        src={publication.user.profile_photo_url || '/default-user.png'} 
                                        alt={publication.user.name}
                                        className="h-12 w-12 rounded-full object-cover"
                                    />
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center space-x-1">
                                            <p className="font-bold truncate">{publication.user.name}</p>
                    ...
                    {!nextPageUrl && publications.length > 0 && !loadingMore && (
